...
detectet= float(robot.get_sonar())
while detect!=0:
        angle=0.05
        velocity=0.01
  #      if distance <= 2:  # and detec != 0:
   #         velocity = 0.0
    #    for distance==59:
 '''    #   maxvelocity=60
def velocity_control(distance, velocity):
    max_distance = max(distance)
    if max_distance == max(distance):
        return max(velocity)
    while max_distance in distance:
        distance.remove(max_distance)
    return velocity[i+1] - velocity[i]
def velocity_control(distance, velocity):

    if maxdistance==max(distance[i]):
        return max(velocity)
